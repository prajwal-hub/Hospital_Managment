...
#for showing signup/login button for admin(by John)
...
#for showing signup/login button for doctor(by John)
...
#for showing signup/login button for patient(by John)
...
def admin_signup_view(request):
    form = forms.AdminSigupForm()  # Ensure this is the correct form name
...
#-----------for checking user is doctor , patient or admin(by John)
def is_admin(user):
...
def admin_add_appointment_view(request):
    ...
    return render(request, 'hospital/admin_add_appointment.html', {
        ...
        'doctors': models.Doctor.objects.all().filter(status=True),  # Add this line
        'patients': models.Patient.objects.all().filter(status=True)  # Add this line
    })
...
#facebook : https://www.facebook.com/mjohnmilton.mjohnmilton.3
#Youtube :youtube.com/@JohnMiltonOfficials
